---
to: packages/<%= h.inflection.dasherize(name.toLowerCase()) %>/package.json
---
{
  ...
  "keywords": [
    "aws",
    "lambda",
    "middleware"<% if(keywords.trim().length > 0) { %>,
    <% } %><%- keywords.trim().split(',').filter(str => str !== '').map(keyword => '"' + keyword.trim() + '"').join(',\n    ') %>
  ],
